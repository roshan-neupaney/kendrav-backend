...
def HasWorkspacePermission(required_permission):
    class _HasWorkspacePermission(BasePermission):
        def has_permission(self, request, view):
            ...
            member_roles = workspace_member.member_roles.all().values_list('role_id', flat=True)
            
            role_permissions = RolePermission.objects.select_related('permission').filter(role_id__in=member_roles)
            ...
            member_permissions = workspace_member.member_permissions.select_related('permission').all()
